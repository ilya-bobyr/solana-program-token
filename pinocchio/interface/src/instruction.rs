...
use {
    ...
    pinocchio::{entrypoint_v2::ProgramDataParser, program_error::ProgramError},
};
...
impl TryFrom<u8> for TokenInstruction {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            0..=24 | 38 | 45 | 255 => Ok(unsafe { transmute::<u8, TokenInstruction>(value) }),
...
// -- Instances below should be autogenerated by macros in the future. --

impl<'data> ProgramDataParser<'data> for TokenInstruction {
    type Error = ProgramError;
...
impl<'data> ProgramDataParser<'data> for AuthorityType {
    type Error = ProgramError;
    ...
    where
       Self: Sized + 'data
    {
        let Some((&authority_type, data)) = data.split_first() else {
            return Err(ProgramError::InvalidInstructionData);
        };
...
mod tests {
    ...
    #[test]
    fn test_token_instruction_program_data_parser_from_u8() {
        for byte in 0..u8::MAX {
            let data = &[byte];
            let actual = <TokenInstruction as ProgramDataParser>::parse(data.as_ref());

            let expected = match TokenInstruction::from_repr(byte.into()) {
                Some(variant) => Ok((variant, &data[1..])),
    ...
    #[test]
    fn test_authority_type_program_data_parser_from_u8() {
        for byte in 0..u8::MAX {
            let data = &[byte];
            let actual = <AuthorityType as ProgramDataParser>::parse(data.as_ref());

            let expected = match AuthorityType::from_repr(byte.into()) {
                Some(variant) => Ok((variant, &data[1..])),
                None => Err(ProgramError::InvalidArgument),
            };
